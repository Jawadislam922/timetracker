                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                {(isProfileRequired) && (
                                    <div>
                                        <label className="block text-sm font-semibold text-slate-300 mb-3">
                                            Upwork Profiles
                                            {isProfileRequired && <span className="text-red-400 ml-1">*</span>}
                                        ...
                                        {selectedProfiles.length > 0 && (
                                            <div className="mb-3 flex flex-wrap gap-2">
                                                {selectedProfiles.map((profile) => (
                                                    <div
                                                        key={profile.id}
                                                        className="inline-flex items-center gap-2 px-4 py-2 bg-purple-500/15 text-purple-300 rounded-lg border-2 border-purple-500/40 font-medium text-sm shadow-sm"
                                                    >
                                                        ...
                                                        <div>
                                                            ...
                                                            {profile.email && <span className="text-xs text-purple-300 ml-1">({profile.email})</span>}
                                                        </div>
                                                        <button
                                                            type="button"
                                                            onClick={() => handleProfileRemove(profile.id)}
                                                            className="hover:bg-purple-500/30 rounded-full p-1 transition-colors ml-1"
                                        ...
                                        <div ref={profileRef} className="relative">
                                            ...
                                            {showProfileDropdown && filteredProfiles.length > 0 && (
                                                <div ref={profileDropdownRef} className="absolute z-50 w-full mt-2 bg-slate-900 border-2 border-slate-800 rounded-xl shadow-xl max-h-64 overflow-y-auto">
                                                    {filteredProfiles.map((profile) => (
                                                        <button
                                                            key={profile.id}
                                                            type="button"
                                                            onClick={() => handleProfileSelect(profile)}
                                                            className="w-full px-4 py-3 text-left hover:bg-slate-800 transition-all border-b border-slate-800 last:border-0 flex items-center gap-3 group"
                                                        >
                                                            ...
                                                            <div className="flex-1">
                                                                <div className="font-semibold text-slate-300">{profile.name}</div>
                                            ...
                                            {filteredProfiles.length === 0 && profileSearch && showProfileDropdown && (
                                                <div className="absolute z-50 w-full mt-2 bg-slate-900 border-2 border-slate-800 rounded-xl shadow-lg">
                                ...
                                {form.data.name && (
                                    <div className="rounded-lg border border-slate-800 bg-slate-950 p-5">
                                        <div className="flex items-center mb-5">
                                            ...
                                            <div>
                                                <p className="text-sm text-purple-300 font-medium mb-1">Client Preview</p>
                                                <p className="font-bold text-slate-100 text-xl">{form.data.name}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                            {form.data.work_type && (
                                                <div>
                                                    <p className="text-sm text-slate-400 font-medium mb-2">Work Type</p>
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold bg-purple-500/15 text-purple-300 rounded-lg border-2 border-purple-500/40 shadow-sm">
                                                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                            ...
                                            {selectedProfiles.length > 0 && (
                                                <div>
                                                    <p className="text-sm text-slate-400 font-medium mb-2">Upwork Profiles ({selectedProfiles.length})</p>
                                                    <div className="flex flex-wrap gap-2">
                                                        {selectedProfiles.map((profile) => (
                                                            <span
                                                                key={profile.id}
                                                                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold bg-pink-500/15 text-pink-300 rounded-lg border-2 border-pink-500/40 shadow-sm"
                                                            >
                                                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                                    <path d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" />
                                                                </svg>
                                        ...
                                        {form.data.tags && form.data.tags.length > 0 && (
                                            <div className="pt-4 border-t border-slate-800">
                                                <p className="text-sm text-slate-400 font-medium mb-2">Tags ({form.data.tags.length})</p>
                                                <div className="flex flex-wrap gap-2">
                                                    {form.data.tags.map((tag, index) => (
                                                        <span
                                                            key={index}
                                                            className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold bg-slate-700/40 text-slate-300 rounded-lg border-2 border-slate-700 shadow-sm"
                                                        >
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-800">
                                    ...
                                    <Link
                                        ...
                                        className="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-slate-900 hover:bg-slate-800 text-slate-200 rounded-lg font-semibold transition-all shadow-md hover:shadow-lg border-2 border-slate-700"
                                    >
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
