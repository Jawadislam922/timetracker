...
function AntivirusHelp() {
    return (
        <div className="mt-5 rounded-lg border border-slate-800 bg-slate-900 p-5 shadow-sm">
            <div className="mb-3 flex items-center gap-2">
                <ShieldAlert className="h-5 w-5 text-amber-600" />
                <h2 className="text-base font-semibold text-white">Antivirus blocking the install or sign-in? (Windows)</h2>
            </div>
            <p className="text-sm leading-6 text-slate-400">
                ...
                <span className="font-medium text-slate-100"> “cannot be closed,”</span> the app gets quarantined, or sign-in
                shows <span className="font-medium text-slate-100"> “Could not reach the server.”</span> Add an exception once
            ...
            <div className="mt-4 rounded-lg bg-slate-900 p-4">
                ...
                <p className="mt-2 text-xs text-slate-400">It’s normally this (paste into the address bar to confirm — older installs may read “Timetracker Desktop”):</p>
                <div className="mt-2">
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
                {/* Bitdefender */}
                <div className="rounded-lg border border-rose-500/40 bg-rose-500/20 p-4">
                    <div className="mb-3 flex items-center gap-2">
                        <ShieldCheck className="h-4 w-4 text-rose-600" />
                        <div className="text-sm font-semibold text-slate-100">Bitdefender Total Security</div>
                    </div>

                    <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">A · Exclude the folder</div>
                    <ol className="ml-4 list-decimal space-y-1 text-sm text-slate-300">
                        ...
                        <li>Paste the install-folder path and turn <span className="font-medium">ON</span> every toggle — especially <span className="font-semibold text-rose-300">Advanced Threat Defense</span> (this one blocks SA&nbsp;Track.exe) and <span className="font-medium">Online Threat Prevention</span>. Click <span className="font-medium">Save</span>.</li>
                    </ol>

                    <div className="mb-1 mt-3 text-xs font-semibold uppercase tracking-wide text-slate-400">B · Allow it through the Firewall</div>
                    <p className="mb-1 text-xs text-slate-400">Required, or sign-in fails with “Could not reach the server.”</p>
                    <ol className="ml-4 list-decimal space-y-1 text-sm text-slate-300">
                        ...
                        <li>Search <span className="font-medium">SA Track</span>. Set the top <span className="font-medium">sa track.exe</span> rule’s <span className="font-medium">Access</span> to <span className="font-semibold text-emerald-300">ON</span> (Any network, Any protocol, Both). Use <span className="font-medium">Add rule</span> if none exists.</li>
                    </ol>

                    <div className="mb-1 mt-3 text-xs font-semibold uppercase tracking-wide text-slate-400">C · If it was already blocked</div>
                    <ol className="ml-4 list-decimal space-y-1 text-sm text-slate-300">
                ...
                {/* Windows Defender */}
                <div className="rounded-lg border border-blue-500/40 bg-blue-500/20 p-4">
                    <div className="mb-3 flex items-center gap-2">
                        <Network className="h-4 w-4 text-blue-600" />
                        <div className="text-sm font-semibold text-slate-100">Windows Security (Defender)</div>
                    </div>
                    ...
                    <div className="mb-1 mt-3 text-xs font-semibold uppercase tracking-wide text-slate-400">B · If it was already quarantined</div>
                    <ol className="ml-4 list-decimal space-y-1 text-sm text-slate-300">
                    ...
                    <div className="mb-1 mt-3 text-xs font-semibold uppercase tracking-wide text-slate-400">C · The blue “Windows protected your PC” box</div>
                    <ol className="ml-4 list-decimal space-y-1 text-sm text-slate-300">
                    ...
                    <p className="mt-3 text-xs text-slate-400">Defender’s firewall allows the app’s outbound connection by default, so no firewall rule is usually needed here.</p>
                </div>
            </div>

            <p className="mt-4 text-xs text-slate-400">
                Also seen on some machines: <span className="font-medium text-slate-300">NordVPN Threat Protection</span> can block the connection too —
    ...
    return (
        <div className="mt-5 rounded-lg border border-slate-800 bg-slate-900 p-5 shadow-sm">
            <div className="mb-3 flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-white">Opening the app on macOS (first launch)</h2>
            </div>
            <p className="text-sm leading-6 text-slate-400">
                The Mac build is not yet signed with an Apple Developer ID, so on first launch macOS shows
                <span className="font-medium text-slate-100"> “Apple could not verify… Not Opened.”</span> This is expected.
                <span className="font-semibold text-rose-400"> Do not click “Move to Bin.”</span> Unblock it once per Mac:
            </p>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
                ...
                <div className="rounded-lg bg-slate-900 p-4">
                    <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Option B — No Terminal</div>
                    <ol className="ml-4 list-decimal space-y-1 text-sm text-slate-300">
    ...
    return (
        <section className="rounded-lg border border-slate-800 bg-slate-900 p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
                ...
                <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${available ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-700/40 text-slate-300'}`}>
                    {available ? 'Available' : 'Pending'}
                </span>
            </div>

            <div className="mt-5 grid gap-3 rounded-lg bg-slate-950 p-4 text-sm text-slate-400 sm:grid-cols-2">
                <div>
                    <div className="text-xs font-semibold uppercase text-slate-500">Version</div>
                    <div className="mt-1 font-medium text-slate-100">{download?.version || 'Not built'}</div>
                </div>
                <div>
                    <div className="text-xs font-semibold uppercase text-slate-500">Size</div>
                    <div className="mt-1 font-medium text-slate-100">{formatBytes(download?.size) || '-'}</div>
                </div>
                <div className="sm:col-span-2">
                    <div className="text-xs font-semibold uppercase text-slate-500">SHA256</div>
                    <div className="mt-1 break-all font-mono text-xs text-slate-300">{download?.sha256 || '-'}</div>
                </div>
            </div>
            ...
            <div className="mt-5">
                {available ? (
                    <a
                        href={download.url}
                        className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-4 py-3 text-sm font-semibold text-slate-200 transition hover:bg-slate-800 sm:w-auto"
                    >
                    ...
                    <button
                        type="button"
                        disabled
                        className="inline-flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-lg bg-slate-800 px-4 py-3 text-sm font-semibold text-slate-500 sm:w-auto"
                    >
                        <Download className="h-4 w-4" />
                        Download
                    </button>
            ...
            <div className="w-full px-4 py-6 sm:px-6 lg:px-8">
                <div className="mb-6 rounded-lg border border-slate-800 bg-slate-900 p-5 shadow-sm">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex items-center gap-2 rounded-lg bg-amber-500/15 px-4 py-3 text-sm text-amber-300">
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                    ...
                    <DownloadCard
                        ...
                        icon={Apple}
                        accent="bg-slate-700 text-slate-300"
                ...
                <div className="mt-5 grid gap-4 lg:grid-cols-3">
                    <div className="rounded-lg border border-emerald-500/40 bg-emerald-500/15 p-4 text-sm text-emerald-200">
