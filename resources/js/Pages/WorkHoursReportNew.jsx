...
export default function WorkHoursReport({ 
    ...
    // Group by user
    const userStats = filteredData.reduce((acc, entry) => {
        const userName = entry.user?.name || 'Unknown';
        if (!acc[userName]) {
            acc[userName] = { hours: 0, entries: 0 };
        }
        acc[userName].hours += Number(entry.hours || 0);
                ...
                <div className="max-w-full mx-auto px-6 lg:px-12 xl:px-16">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-100">
                            ...
                            <p className="text-slate-600 text-sm font-medium mb-1">Total Hours</p>
                            <p className="text-3xl font-bold text-slate-900">{timeFormat(totalHours.toFixed(2))}</p>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-100">
                            ...
                            <p className="text-slate-600 text-sm font-medium mb-1">Active Clients</p>
                            <p className="text-3xl font-bold text-slate-900">{uniqueClientsCount}</p>
                        </div>
                    </div>
                    ...
                    {Object.keys(userStats).length > 0 && (
                        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6 border border-slate-100">
                            <h2 className="text-xl font-bold text-slate-900 mb-4">Hours by User</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {Object.entries(userStats)
                                    .sort(([, a], [, b]) => b.hours - a.hours)
                                    .map(([userName, stats]) => (
                                        <div key={userName} className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-4 border border-emerald-200">
                                            ...
                                            <div className="text-2xl font-bold text-emerald-600">
                                                {timeFormat(stats.hours.toFixed(2))}
                                            </div>
                                        </div>
                                    ))}
                            </div>
